...
int ImGui::TypingSelectFindMatch(ImGuiTypingSelectRequest* req, int items_count, const char* (*get_itename_func)(void*, int), void* user_data, int nav_iteidx)
{
    ...
    if (req->SingleCharMode && (req->Flags & ImGuiTypingSelectFlags_AllowSingleCharMode))
        idx = TypingSelectFindNextSingleCharMatch(req, items_count, get_itename_func, user_data, nav_iteidx);
    else
        idx = TypingSelectFindBestLeadingMatch(req, items_count, get_itename_func, user_data);
    if (idx != -1)
        SetNavCursorVisibleAfterMove();
    return idx;
}
...
int ImGui::TypingSelectFindNextSingleCharMatch(ImGuiTypingSelectRequest* req, int items_count, const char* (*get_itename_func)(void*, int), void* user_data, int nav_iteidx)
{
    // FIXME: Assume selection user data is index. Would be extremely practical.
    //if (nav_iteidx == -1)
    //    nav_iteidx = (int)g.NavLastValidSelectionUserData;

    int first_match_idx = -1;
    bool return_next_match = false;
    for (int idx = 0; idx < items_count; idx++)
    {
        const char* itename = get_itename_func(user_data, idx);
        if (ImStrimatchlen(req->SearchBuffer, req->SearchBuffer + req->SingleCharSize, itename) < req->SingleCharSize)
            continue;
        if (return_next_match)                           // Return next matching item after current item.
            return idx;
        if (first_match_idx == -1 && nav_iteidx == -1) // Return first match immediately if we don't have a nav_iteidx value.
            return idx;
        if (first_match_idx == -1)                       // Record first match for wrapping.
            first_match_idx = idx;
        if (nav_iteidx == idx)                         // Record that we encountering nav_item so we can return next match.
            return_next_match = true;
    }
    return first_match_idx; // First result
}

int ImGui::TypingSelectFindBestLeadingMatch(ImGuiTypingSelectRequest* req, int items_count, const char* (*get_itename_func)(void*, int), void* user_data)
{
    ...
    {
        const char* itename = get_itename_func(user_data, idx);
        const int match_len = ImStrimatchlen(req->SearchBuffer, req->SearchBuffer + req->SearchBufferLen, itename);
